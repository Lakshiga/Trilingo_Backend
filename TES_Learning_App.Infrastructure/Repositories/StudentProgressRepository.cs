...
namespace TES_Learning_App.Infrastructure.Repositories
{
    public class StudentProgressRepository : IStudentProgressRepository
    {
        ...
        public async Task<StudentProgress> AddAsync(StudentProgress progress)
        {
        ...
        public Task DeleteAsync(StudentProgress progress)
        {
            _context.StudentProgresses.Remove(progress);
            return Task.CompletedTask;
        }

        public async Task<double> GetAverageScoreByStudentAsync(Guid studentId)
        {
            var average = await _context.StudentProgresses
                .Where(sp => sp.StudentId == studentId && sp.MaxScore > 0)
                .Select(sp => (double)sp.Score / sp.MaxScore * 100)
                .AverageAsync();

            return average;
        }

        public async Task<int> GetTotalCompletedByStudentAsync(Guid studentId)
        {
            return await _context.StudentProgresses
        ...
        public async Task<int> GetTotalAttemptedByStudentAsync(Guid studentId)
        {
            return await _context.StudentProgresses
                .Where(sp => sp.StudentId == studentId)
